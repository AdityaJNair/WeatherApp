...
function updateUsingCurrentPosition(latitude, longitude) {
    $.ajax({
        ...
        success: function (data) {
            var weatherInformation = new WeatherInfo(Math.round(data.main.temp_min) - 273 + "°C", Math.round(data.main.temp_max) - 273 + "°C", "", "", data.weather[0].main, data.weather[0].description, data.weather[0].icon, data.main.humidity, Math.round(data.main.temp) - 273 + "°C", toTitleCase(data.name), getPosition(data.wind.deg), data.sys.country, data.main.pressure + " hpa", formatTime(data.sys.sunrise), formatTime(data.sys.sunset), data.wind.speed + " m/s", data.coord.lat, data.coord.lon);
    ...
    $.ajax({
        ...
        success: function (data) {
            var weatherInfoD2 = new WeatherInfoMultiple(Math.round(data.list[8].main.temp_min) - 273 + "°C", Math.round(data.list[8].main.temp_max) - 273 + "°C", formatDate(data.list[8].dt_txt), formatHeaderTime(data.list[8].dt_txt), data.list[8].weather[0].main, data.list[8].weather[0].description, data.list[8].weather[0].icon);
            var weatherInfoD3 = new WeatherInfoMultiple(Math.round(data.list[16].main.temp_min) - 273 + "°C", Math.round(data.list[16].main.temp_max) - 273 + "°C", formatDate(data.list[16].dt_txt), formatHeaderTime(data.list[16].dt_txt), data.list[16].weather[0].main, data.list[16].weather[0].description, data.list[16].weather[0].icon);
            var weatherInfoD4 = new WeatherInfoMultiple(Math.round(data.list[24].main.temp_min) - 273 + "°C", Math.round(data.list[24].main.temp_max) - 273 + "°C", formatDate(data.list[24].dt_txt), formatHeaderTime(data.list[24].dt_txt), data.list[24].weather[0].main, data.list[24].weather[0].description, data.list[24].weather[0].icon);
            var weatherInfoD5 = new WeatherInfoMultiple(Math.round(data.list[32].main.temp_min) - 273 + "°C", Math.round(data.list[32].main.temp_max) - 273 + "°C", formatDate(data.list[32].dt_txt), formatHeaderTime(data.list[32].dt_txt), data.list[32].weather[0].main, data.list[32].weather[0].description, data.list[32].weather[0].icon);
...
var formatTime = function (unixTimestamp) {
    ...
    if (hours > 12) {
        var hoursRemaining = hoursRemaining = 24 - hours;
...
var WeatherInfo = (function () {
    function WeatherInfo(tempmin, tempmax, date, time, maindescription, description, icon, humidity, temperature, place, windDirection, country, pressure, sunrise, sunset, wind, latitude, longitude) {
...
var WeatherInfoMultiple = (function (_super) {
    __extends(WeatherInfoMultiple, _super);
    function WeatherInfoMultiple(tempmin, tempmax, date, time, maindescription, description, icon) {
        _super.call(this, tempmin, tempmax, date, time, maindescription, description, icon, "", "", "", "", "", "", "", "", "", "", "");
    }
